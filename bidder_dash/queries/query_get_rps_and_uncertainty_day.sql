...
with status_fix as (
    ...
    select timestamp_trunc(date_hour, day) date, {select_dimensions},
    ...
    select date, {group_by_dimensions},
        sum(session_count) over(partition by {group_by_dimensions} order by date rows between {N_days_preceding} preceding and current row) session_count,
        sum(sum_revenue) over(partition by {group_by_dimensions} order by date rows between {N_days_preceding} preceding and current row) sum_revenue,
        sum(sum_revenue_sq) over(partition by {group_by_dimensions} order by date rows between {N_days_preceding} preceding and current row) sum_revenue_sq
    from agg
