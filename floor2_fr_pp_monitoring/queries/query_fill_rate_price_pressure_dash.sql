...
hourly_analytics as (
    ...
    select date_hour,
        ...
        (safe_divide(COALESCE(SAFE_DIVIDE(SUM(if(baseline, revenue, 0)), SUM(if(baseline, requests, 0))), 0),
            COALESCE(SAFE_DIVIDE(SUM(if(optimised, revenue, 0)), SUM(if(optimised, requests, 0))), 0)) - 1) * 100 price_pressure,
...
stats as (
    select *,
        ...
        avg(price_pressure) over(order by date_hour rows between {N} preceding and current row) price_pressure_sm,
