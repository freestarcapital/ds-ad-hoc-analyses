...
with base as (
    ...
    group by fs_testgroup, bidder, device_category, rtt_category, fsrefresh, status
), results_expt as (
    select *
    from base
    ...
    select bidder, 'all' country_code, device_category, rtt_category, fsrefresh, sum(session_count) session_count
    from base
    where fs_testgroup = 'optimised'
    group by bidder, device_category, rtt_category, fsrefresh
), results_opt_winning_rps as (
    select b.* except (status, revenue, session_count)
    from base b
    join results_expt r using (bidder, device_category, rtt_category, fsrefresh, status)
    ...
    from results_expt e
    join results_opt o using (bidder, device_category, rtt_category, fsrefresh)
    left join results_opt_winning_rps o_r using (bidder, device_category, rtt_category, fsrefresh)
)
select * from results;
