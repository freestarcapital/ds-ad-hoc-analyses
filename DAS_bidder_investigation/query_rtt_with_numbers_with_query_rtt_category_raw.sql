...
CREATE OR REPLACE TABLE `sublime-elixir-273810.ds_experiments_us.DAS_bidder_inv_rtt_raw_{processing_date}` AS
...
new_rps as(
  select bidder, device_category,country_code,rtt_category,status,prev,rtt_rank,rtt_category_raw,
...
final_rtt as (
  select bidder, device_category,country_code,rtt_category,status,rtt_rank,rtt_category_raw,
  revenue,session_count, safe_divide(revenue,session_count)*1000 avg_rps,
...
all_cohorts as (
  select bidder, device_category, country_code, rtt_category, rtt_category_raw, status,rtt_rank, avg_rps,fallback_rps  from single_entry_cohorts
  union all
  select bidder, device_category, country_code, rtt_category, rtt_category_raw, status,rtt_rank, avg_rps,fallback_rps  from final_rtt
  ),
...
reverse as (
  ...
  rtt_category,rtt_rank,rtt_category_raw,
  status, avg_rps,fallback_rps
  from all_cohorts
  )

select r.bidder, r.device_category, r.country_code, r.rtt_category_raw,
case when min(rtt_rank) over (partition by device_category,country_code,bidder) = rtt_rank then 'default'
