...
def get_domains_from_collection_ids(collection_ids, start_date=dt.date.today()-dt.timedelta(days=23), end_date=dt.date.today()):
    min_page_hits = 10000
    ...
    repl_dict = {'start_date': start_date.strftime("%Y-%m-%d"),
                 ...
                 'collection_ids_list': "('" + "', '".join(collection_ids) + "')",
...
def get_domains_from_test_names(test_names, start_date=dt.date.today()-dt.timedelta(days=23), end_date=dt.date.today()):

    print(f'searching for domains for test_names: {", ".join(test_names)} from {start_date} to {end_date}')

    repl_dict = {'start_date': start_date.strftime("%Y-%m-%d"),
                 'end_date': end_date.strftime("%Y-%m-%d"),
                 'test_names_list':  "('" + "', '".join(test_names) + "')"}
...
def does_table_exist(tablename):
    query = f"select count(*) from {dataset_name}.INFORMATION_SCHEMA.TABLES where table_catalog || '.' || table_schema || '.' || table_name = '{tablename}'"
...
def main(force_recreate_table=False):
    ...
    #TIMEOUTS
    name = 'timeouts'
    datelist = pd.date_range(start=dt.date(2025,9,13), end=dt.date(2025,9,15))
    # test_domains = get_domains_from_collection_ids(['9c42ef7c-2115-4da9-8a22-bd9c36cdb8b4'])
    test_domains = get_domains_from_collection_ids(['5b60cd25-34e3-4f29-b217-aba2452e89a5'])
    ...
    for date in datelist.tolist():
        ...
        create_or_insert_statement = (
            f"delete from `{tablename}` where date = '{date.strftime('%Y-%m-%d')}'; "
...
def main_data_explore():
    ...
    query = open(os.path.join(sys.path[0], f"queries/{query_filename}.sql"), "r").read()
    df = get_bq_data(query,repl_dict)
    df.transpose().to_csv('AB_data_8.csv')
