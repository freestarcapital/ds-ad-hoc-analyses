...
where timestamp_trunc(_PARTITIONTIME, day) >= timestamp_sub(timestamp_trunc(current_timestamp(), day), interval 1 day)
...
select record_date__mst , test_name , test_group , session_id , n
from (
select distinct record_date__mst , test_name , test_group , session_id ,
...
select record_date__utc , record_date__mst , test_name , test_group , session_id , requests , n
from ( select record_date__utc , record_date__mst , test_name , test_group , session_id , requests ,
...
where date(_PARTITIONDATE) >= current_date() - 1 and date(_PARTITIONDATE) < current_date()
and test_name is not null
group by 1,2,3,4,5,6 )
...
'prebid' as inventory_platform , bwr.session_id ,
...
inner join auction_end_raw__test__bwr aer on aer.record_date__utc = bwr.record_date__utc
 ...
 and aer.test_group = bwr.test_group group by 1,2,3,4,5,6,7 )
 ...
 and l.date = m.EventDateMST where m.EventDateMST >= current_date() - 1
 and m.EventDateMST < current_date() and
 ...
 where EventDateMST >= current_date() - 1 and EventDateMST < current_date()
 ...
 'us_gam_dtf__amazon_adx_ebda' as inventory_platform , m.session_id ,
 ...
 and aer.session_id = m.session_id group by 1,2,3,4,5,6 )

 -- union both bwr and dtf
 , bwr_us_gam_dtf__test as (
 select record_date__mst , site_id , test_name , test_group , inventory_platform , requests , session_id ,
 ...
 , final as (
 select m.record_date__mst , m.site_id , m.test_name , m.test_group , ab.collection_id , ab.methodology ,
