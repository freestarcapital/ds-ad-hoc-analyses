...
UNION ALL
  ...
  SELECT domain, session_id, rev / (imps+unfilled) * 1000 cpma
  FROM ad_requests
  where imps+unfilled > 0
  qualify row_number() over(partition by domain, session_id order by EventTimestamp) = 1
)
